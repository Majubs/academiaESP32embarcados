...
static void event_handler(void *arg, esp_event_base_t event_base, int32_t event_id, void *event_data)
{
    ...
    { // if wifi event and wifi event station start
        esp_wifi_connect();
    }
    else if (event_base == WIFI_EVENT && event_id == WIFI_EVENT_STA_DISCONNECTED)
    { // if wifi event and wifi event station disconnected
        ...
        if (attempt_reconnect)
        {
                ...
                wifi_event_sta_disconnected->reason == WIFI_REASON_AUTH_EXPIRE)   // if wifi event data reason is auth expire
            {
                ...
                {                                                // if retry number is less than maximum retry
    ...
    {                                                               // if ip event and ip event station got ip
...
static void ap_event_handler(void *arg, esp_event_base_t event_base, int32_t event_id, void *event_data)
{
    ...
    {                                                                                     // if wifi event ap station connected
    ...
    {                                                                                           // if wifi event ap station disconnected
